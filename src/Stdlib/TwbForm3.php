...
class TwbForm3 {
		...
		?>>
				  ...
				  foreach ($form->getElements() as $element) {
					...
					<fieldset id="<?= $element->attributes->id ?>" <?= $element->parseAttributes() ?>>
						...
						if (!$element->options->label) echo $element->getMultipleButton($fieldset);
						echo $fieldset;
						?>
					</fieldset>
					<?php
		...
		if ($element->type !== 'hidden'):
			...
			?>">
				 ...
				 if ($element->options->label):
					 ...
					 if (is_object($label)) {
						 ...
						 if ($label->attrs) $attrs = Util::parseAttrArray($label->attrs->toArray(), array('class'));
						 $label = $label->text;
					 }
					 ?>
			...
			if (!in_array($element->type, array('hidden'))):
				...
				if ($element->options->containerAttrs)
						$attrs = Util::parseAttrArray($element->options->containerAttrs->toArray(), array('class'));
				?>
	...
	public static function inlineElement(Element $element) {
		if (!in_array($element->type, array('checkbox', 'radio', 'submit', 'button', 'reset', 'hidden'))) {
			?>
			...
			if (!in_array($element->type, array('checkbox', 'radio', 'submit', 'button', 'reset'))) {
				?>
			</div>
			<?php
		}
