...
class Engine {
	...
	protected static function checkAlias($module) {
		...
		if (array_key_exists($module, static::getConfig('modules'))) return $module;

		// check module alias
		foreach (static::getConfig('modules') as $cModule => $moduleOptions) {
			if (!array_key_exists('alias', $moduleOptions)) continue;

			if ($moduleOptions['alias'] === Util::camelToHyphen($module)) return $cModule;
		}
	...
	public static function getAction($exception = true) {
		...
		if (!empty($urls) && count($urls) >= 3) {
			...
			if (!$return = static::getDefaultAction(static::getModule(), false)) {
				if ($exception) throw new \Exception('Required action not found');
				return '-';
			}
		}
	...
	public static function run(array $config) {
		...
		if ($cache && $out = $cache->fetch($name)) {
			...
			$view->render();
			$data = ob_get_clean();
			if ($cache && !Session::fetch('noCache')) $cache->save($name, $data);
			echo $data;
			static::terminate();
		}
	}
