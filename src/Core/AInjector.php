...
abstract class AInjector extends ACore {
     ...
     */
    final public function __construct($inject = true) {
        if ($inject) {
            $this->doInject();
        }
        $this->construct();
    ...
    final private static function addInjecting($className) {
        if (in_array($className, self::$injecting)) {
            if (self::$injecting[count(self::$injecting) - 1] === $className)
                return true;
        ...
        if (is_array($this->inject())) {
            foreach ($this->inject() as $classArray) {
                if (!is_array($classArray) ||
                        ...
                        (is_array($classArray) && !class_exists($classArray['class'])))
                    continue;

                if ($classArray['class'] === $className)
                    return false;

                if (!$this->canInject($classArray, $className))
                    return false;
            }
        }
