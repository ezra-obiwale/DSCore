...
class Authenticate {
	...
	private function checkPermission(array $properties) {
		...
		if (isset($properties['actions']) && !empty($properties['actions']) &&
				...
				!in_array(\Util::camelToHyphen($this->action), $properties['actions'])) return null;
		...
		foreach ($properties as $ppt => $values) {
			...
			if (strtolower($ppt) !== 'actions') {
				if ($this->userId->getUser()->$ppt === null) return false;

				if (empty($values)) return true;

				if (!in_array($this->userId->getUser()->$ppt, $values)) return false;
			}
		}
	...
	final public function execute() {
		...
		foreach ($this->controller->accessRules() as $rule) {
			...
			if (strtolower($permission) === 'allow') {
				if ($this->checkPermission($rules) === TRUE) return true;
			}
			elseif (strtolower($permission) === 'deny') {
				if ($this->checkPermission($rules) === TRUE) return false;
			}
		}
