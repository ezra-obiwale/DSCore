...
class View {
	...
	final public function getViewFile($addOthers = true) {
		if ($addOthers && is_array($this->viewFile)) {
			...
			if (!isset($this->viewFile[2])) $this->viewFile[2] = Util::camelToHyphen(engineGet('action'));
	 ...
	 * @return string
	 */
	final public function getOutput($module, $controller, $action, array $params = array(),
								 $partial = true) {
		$view = new View();
