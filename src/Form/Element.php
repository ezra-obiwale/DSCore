...
class Element extends Object {
	...
	public function __construct(array $data = array(), $preserveArray = false, $preserveKeyOnly = null) {
		...
		if (empty($this->options->value) && empty($this->options->values)) $this->options->value = null;
	...
	public function prepareInfo($infoType = self::ALL_INFO) {
		$return = '';
		if ($this->options->inlineInfo && in_array($infoType, array(self::INLINE_INFO, self::ALL_INFO)))
				$return .= '<span class="help-inline">' . $this->options->inlineInfo . '</span>';
		if (($this->options->blockInfo && in_array($infoType, array(self::BLOCK_INFO, self::ALL_INFO))) || ($this->errors &&
	...
	public function render() {
		if ($this->parent) {
			if (!static::$count[$this->parent]) static::$count[$this->parent] = 0;
